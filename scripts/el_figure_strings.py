...
CHROME = {
    ...
    "% of the same 16 indicators": "% των ίδιων 16 δεικτών",
...
QUESTIONS = {
    ...
    "How much accumulated unemployment has each country absorbed, and where "
    "does Greece sit?":
        "Πόση συσσωρευμένη ανεργία έχει απορροφήσει κάθε χώρα, και πού "
        "βρίσκεται η Ελλάδα;",
...
CAVEATS = {
    "F1":
        "Περιγραφική σύγκριση δύο επίσημων δεικτών. Στην οικονομική δυσκολία "
        "απαντούν ΝΟΙΚΟΚΥΡΙΑ, ενώ η εισοδηματική φτώχεια μετρά ΑΤΟΜΑ, γι' "
        "αυτό ο άξονας λέει «ποσοστό» και όχι το ένα ή το άλλο. Στη δεύτερη "
        "καρτέλα η γραμμή προσαρμογής εξαιρεί την Ελλάδα: περιγράφει το "
        "ευρωπαϊκό μοτίβο με το οποίο συγκρίνεται η Ελλάδα, όχι ένα μοτίβο "
        "που διαμόρφωσε και η ίδια. Το σημείο με την ένδειξη δείχνει τη "
        "μεσαία χώρα σε κάθε δείκτη ΞΕΧΩΡΙΣΤΑ, οπότε είναι σημείο αναφοράς "
        "και όχι υπαρκτή χώρα: καμία χώρα δεν βρίσκεται απαραίτητα εκεί. Και "
        "οι δύο καρτέλες αφορούν επίπεδο χώρας και δεν λένε τίποτα για "
        "μεμονωμένα νοικοκυριά.",
    "F8":
        "Επιβεβαίωση από την ίδια έρευνα, όχι ανεξάρτητη επικύρωση ούτε "
        "αιτιώδης απόδειξη. Ελληνικές συσχετίσεις με την οικονομική δυσκολία: "
        "απρόοπτα έξοδα 0,92 · υλική στέρηση 0,94 · θέρμανση του σπιτιού 0,87 "
        "· καθυστερήσεις σε λογαριασμούς 0,37. Κάθε γραμμή είναι απόσταση από "
        "τον δικό της μέσο όρο 2015-2024, μαζί και η ευρωπαϊκή διάμεση τιμή, "
        "ώστε τρεις σειρές με διαφορετικές κλίμακες να μοιράζονται έναν "
        "άξονα: συγκρίνονται τα σχήματα, όχι τα επίπεδα. Η κλίμακα είναι η "
        "ίδια και στις τέσσερις καρτέλες. Η ευρωπαϊκή διάμεση δυσκολία "
        "απουσιάζει σκόπιμα, γιατί το ερώτημα εδώ είναι αν η συγκεκριμένη "
        "δυσκολία κινήθηκε μαζί με τις ελληνικές απαντήσεις, όχι πώς "
        "συγκρίνεται η Ελλάδα.",
    "F21":
        "Περιγραφική σύνοψη της κατάστασης, όχι προβλεπτικός παράγοντας γι' "
        "αυτήν. ΠΕΡΙΓΡΑΦΙΚΟ ΜΟΝΟ. Η εξάπλωση εξετάστηκε ως προβλεπτικός "
        "παράγοντας της οικονομικής δυσκολίας και δεν αντέχει: από μόνη της "
        "δεν φτάνει το όριο (p = 0,12), και μόλις μπουν στο μοντέλο οι "
        "υπόλοιποι συσσωρευμένοι δείκτες, αντιστρέφεται το πρόσημό της. "
        "Συνοψίζει την κατάσταση, δεν την εξηγεί. ΤΟ ΚΑΛΑΘΙ ΕΙΝΑΙ ΣΤΑΘΕΡΟ: "
        "είναι οι 16 δείκτες με έγκυρη ευρωπαϊκή θέση και το 2008 και το "
        "2024, επιλεγμένοι χωρίς να ληφθεί υπόψη αν βελτιώθηκαν ή "
        "χειροτέρεψαν, ώστε να μετριούνται τα ίδια πράγματα στα δύο άκρα και "
        "σε κάθε ενδιάμεσο έτος. Η ανεργία, η ανεργία των νέων και το ποσοστό "
        "απασχόλησης απουσιάζουν επειδή η συγκρίσιμη ευρωπαϊκή κάλυψη ξεκινά "
        "το 2009· οι ώρες εργασίας, η ωριαία αμοιβή, η πίεση της έντασης "
        "εργασίας και οι πραγματικοί μισθοί υπάρχουν όλα, οπότε το καλάθι δεν "
        "είναι χωρίς πληροφορία για την αγορά εργασίας. Ο δείκτης-αποτέλεσμα "
        "και κάθε μεταβλητή ελέγχου του μοντέλου εξαιρούνται. Η ΔΙΑΜΕΣΗ ΤΙΜΗ "
        "ΤΩΝ ΧΩΡΩΝ ΤΗΣ ΕΕ είναι η διάμεσος των ποσοστών των 27 κρατών-μελών "
        "σε αυτό το καλάθι, δηλαδή διάμεσος μεταξύ χωρών και όχι το "
        "σταθμισμένο με τον πληθυσμό ευρωπαϊκό άθροισμα της Eurostat, γι' "
        "αυτό και ονομάζεται έτσι αντί για σκέτο «ΕΕ». Η Κροατία δεν φτάνει "
        "ποτέ πλήρη κάλυψη και στους 16 δείκτες σε καμία χρονιά του καλαθιού "
        "και γι' αυτόν τον λόγο δεν εμφανίζεται ως γραμμή. Ο άξονας της "
        "καρτέλας «Ποιοι δείκτες» δείχνει ΘΕΣΗ, όχι τιμή: οι δείκτες δεν "
        "έχουν κοινή μονάδα μέτρησης, οπότε το 0 είναι η καλύτερη θέση στην "
        "Ένωση για τον συγκεκριμένο δείκτη και το 100 η χειρότερη, όποια "
        "κατεύθυνση κι αν σημαίνει «χειρότερα».",
    "F3":
        "Η σειρά με το αγκυρωμένο όριο είναι προσέγγιση που κατασκευάστηκε "
        "για αυτή την ανάλυση και σημειώνεται ως τέτοια όπου κι αν "
        "εμφανίζεται. Η πρώτη καρτέλα μετρά ΑΤΟΜΑ, η δεύτερη ΕΥΡΩ. Στη "
        "δεύτερη, και οι δύο γραμμές είναι το ίδιο επίσημο όριο: η μία όπως "
        "δημοσιεύτηκε στα χρήματα κάθε χρονιάς, η άλλη μετατρεμμένη σε αυτό "
        "που μπορούσε να αγοράσει το 2008. Η γραμμή σε τρέχουσες τιμές "
        "ανακάμπτει, η γραμμή της αγοραστικής δύναμης όχι, και αυτή η διαφορά "
        "είναι ο λόγος που αποκλίνουν οι δύο μετρήσεις της πρώτης καρτέλας. "
        "Και οι δύο καρτέλες αφορούν μόνο την Ελλάδα και δεν στηρίζουν καμία "
        "σύγκριση μεταξύ χωρών.",
    "F5":
        ...
        "υπάρχει συγκρίσιμη εθνική σειρά για την καρτέλα των συστατικών· η "
    ...
    "F7":
        "ΕΝΑ ΧΑΣΜΑ ΠΟΥ ΣΤΕΝΕΥΕΙ ΔΕΝ ΣΗΜΑΙΝΕΙ ΟΤΙ Η ΕΛΛΑΔΑ ΒΕΛΤΙΩΘΗΚΕ. Ένα "
        "χάσμα μπορεί να κλείσει επειδή η Ελλάδα πλησίασε, επειδή η υπόλοιπη "
        "ΕΕ χειροτέρεψε, ή επειδή όλοι κινήθηκαν προς την ίδια κατεύθυνση με "
        "διαφορετική ταχύτητα. Το γράφημα μετρά ΣΧΕΤΙΚΗ ΣΥΓΚΛΙΣΗ, όχι εθνική "
        "ανάκαμψη, γι' αυτό και τα δύο άκρα, για την Ελλάδα και για τη "
        "διάμεση τιμή της ΕΕ, υπάρχουν στην ένδειξη που εμφανίζεται με το ποντίκι και στον πίνακα. Το "
        "«έκλεισε το 71% του χάσματος» και το «οι συνθήκες βελτιώθηκαν κατά "
        "71%» είναι εντελώς διαφορετικοί ισχυρισμοί. Το μέγεθος που "
        "σχεδιάζεται δεν έχει μονάδα μέτρησης, επειδή τα υποκείμενα χάσματα "
        "μετριούνται σε ποσοστιαίες μονάδες, σε μονάδες δείκτη και σε ΜΑΔ ανά "
        "άτομο, και δεν χωρούν σε κοινό άξονα.",
    "F11":
        "συσχέτιση μεταξύ χωρών | καμία αποδεδειγμένη δυναμική εντός της "
        "Ελλάδας | κανένας αιτιώδης ισχυρισμός | οι σημερινές συνθήκες δεν "
        "αποκλείονται Οι τρεις δείκτες έχουν διαφορετικές μονάδες μέτρησης "
        "και δεν συγκρίνονται μεταξύ τους: η συσσωρευμένη ανεργία και η "
        "επιδείνωση της στέγασης μετριούνται σε μονάδες-έτη, ενώ η μη "
        "ανάκαμψη των μισθών σε συνεχόμενα χρόνια.",
    "F14":
        "ΚΑΜΙΑ από τις δύο εκδοχές του μοντέλου δεν είναι οριστική. Δεν "
        "επιτρέπεται να συγχωνευτούν ούτε να πάρουμε τον μέσο όρο τους, ούτε "
        "να γίνει επιλογή με βάση το μέγεθος του υπολοίπου.",
}

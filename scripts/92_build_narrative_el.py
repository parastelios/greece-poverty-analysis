...
def localize_figure(fid, fig_html):
    ...
    fig_html = re.sub(
        ...
        fig_html, flags=re.S)
    # the always-visible "what this shows" line, replaced wholesale like the
    # methods note just below: a short authored sentence, not a vocabulary of
    # recurring fragments. Figures with no Greek definition yet (not part of
    # this pass) keep rendering nothing, same as the English source.
    if fid in ELF.DEFINITIONS:
        fig_html = re.sub(
            r'<p class="fig-def">.*?</p>',
            '<p class="fig-def"><strong>Τι δείχνει αυτό.</strong> '
            + ELF.DEFINITIONS[fid] + "</p>",
...
{context_el('CTX-11', "επιπλέον στοιχεία από άλλη πηγή",
    "Ανεξάρτητη έρευνα (βαρόμετρο Ipsos/Secours Populaire)",
    '''<p>Μια ανεξάρτητη διεθνής έρευνα καταλήγει σε παρόμοιο συμπέρασμα. Το
    βαρόμετρο της Ipsos για τον γαλλικό οργανισμό Secours Populaire ρώτησε
    το 2026 10.000 ανθρώπους σε 10 χώρες πώς αξιολογούν την οικονομική τους
    κατάσταση: το 44% των Ελλήνων ερωτηθέντων τη χαρακτήρισε «επισφαλή»
    (précaire), έναντι μέσου όρου 29% στις δέκα χώρες. Πρόκειται για
    διαφορετική ερώτηση, διαφορετική κλίμακα και διαφορετικό δείγμα από τον
    δείκτη EU-SILC παραπάνω, οπότε οι δύο αριθμοί δεν διαβάζονται μαζί.
    Αποτελεί όμως ανεξάρτητη ένδειξη ότι η δηλωμένη οικονομική δυσφορία
    στην Ελλάδα βρίσκεται σε ακραίο σημείο σε σχέση με τις άλλες χώρες,
    χωρίς να αναπαράγει το εύρημα αυτής της ανάλυσης.</p>''',
    "Η ίδια έρευνα δείχνει ότι το 44% των Ελλήνων ερωτηθέντων χαρακτηρίζει "
    "την οικονομική του κατάσταση «επισφαλή», έναντι μέσου όρου 29% στις "
    "δέκα συμμετέχουσες χώρες, δεύτερη χειρότερη θέση, πίσω μόνο από τη "
    "Μολδαβία. Το εύρημα συμφωνεί, από ανεξάρτητη πηγή, με το ότι η Ελλάδα "
    "κατατάσσεται 1η από τις 27 χώρες της ΕΕ ως προς τη δηλωμένη οικονομική "
    "δυσκολία.",
    "Τα ποσοστά 44%/29% της έρευνας δεν ισοδυναμούν ούτε συγκρίνονται "
    "απευθείας με τα δικά μας ποσοστά EU-SILC 66,7%/17,1%: προέρχονται από "
    "διαφορετική ερώτηση, διαφορετική κλίμακα απάντησης και διαφορετικό "
    "δείγμα πληθυσμού. Ο «ευρωπαϊκός μέσος όρος» της έρευνας δεν είναι "
    "μέσος όρος ΕΕ-27 της Eurostat: είναι ο μη σταθμισμένος μέσος όρος 10 "
    "εθνικών δειγμάτων των 1.000 ατόμων το καθένα, εκ των οποίων τρία "
    "(Ηνωμένο Βασίλειο, Μολδαβία, Σερβία) δεν είναι κράτη μέλη της ΕΕ. Η "
    "έρευνα βασίζεται σε δείγμα ποσόστωσης μέσω διαδικτυακής έρευνας σε "
    "εγγεγραμμένους χρήστες, όχι σε πιθανοθεωρητική δειγματοληψία όπως το "
    "EU-SILC. Δεν αποτελεί απόδειξη "
    "για την αιτία του χάσματος των 52,6 μονάδων: μετρά μόνο αντίληψη και "
    "δεν ελέγχθηκε ποτέ έναντι κανενός μοντέλου αυτής της ανάλυσης.",
...
{fig('F8', caption="Η δηλωμένη δυσκολία κινείται μαζί με τη συγκεκριμένη οικονομική πίεση")}
...
{context_el('CTX-12', "επιπλέον στοιχεία από άλλη πηγή",
    "Ανεξάρτητη έρευνα (ΕΕΕΠ, προβληματικός τζόγος ανά οικονομική "
    "κατάσταση)",
    '''<p>Μια ανεξάρτητη ελληνική έρευνα εντοπίζει ένα συναφές
    συμπεριφορικό μοτίβο. Η ΕΕΕΠ, ο ρυθμιστής του τζόγου, αναφέρει ότι
    το 9% των ενηλίκων που περιγράφουν την οικονομική τους κατάσταση
    ως δύσκολη εμφανίζει προβληματική συμπεριφορά τζόγου, έναντι 2%
    όσων την περιγράφουν ως άνετη· διαφορετικός πληθυσμός και
    διαφορετικό εργαλείο μέτρησης από τα στοιχεία του EU-SILC
    παραπάνω, αλλά η κατεύθυνση είναι η ίδια: η δηλωμένη
    οικονομική πίεση κινείται μαζί με μια συγκεκριμένη, εξωτερικά
    παρατηρήσιμη συμπεριφορά.</p>''',
    "Μια ανεξάρτητη ελληνική έρευνα, άσχετη με το EU-SILC, βρίσκει ότι "
    "το 9% των ενηλίκων που περιγράφουν την οικονομική τους κατάσταση "
    "ως δύσκολη αναφέρουν προβληματική συμπεριφορά τζόγου, έναντι 2% "
    "όσων την περιγράφουν ως άνετη, μια αναλογία περίπου 4,5 φορών. "
    "Πρόκειται για διαφορετικό πληθυσμό, εργαλείο μέτρησης και φορέα "
    "από τα στοιχεία υλικής δυσκολίας του EU-SILC σε αυτή την "
    "ανάλυση, αλλά συμφωνεί με αυτό το εύρημα ως προς την "
    "κατεύθυνση: η δηλωμένη οικονομική δυσκολία συμβαδίζει με μια "
    "συγκεκριμένη, εξωτερικά παρατηρήσιμη συμπεριφορά.",
    "Δεν αποτελεί απόδειξη για την αιτία του χάσματος των 52,6 "
    "μονάδων, ούτε έλεγχο που έγινε σε αυτή την ανάλυση: ο κίνδυνος "
    "προβληματικού τζόγου δεν εξετάστηκε ποτέ ως υποψήφια μεταβλητή "
    "στους επιβεβαιωτικούς ελέγχους αυτού του έργου. Η αναλογία 9% "
    "προς 2% δεν αποδεικνύει αιτιότητα προς καμία κατεύθυνση: η "
    "οικονομική πίεση θα μπορούσε να οδηγεί σε τζόγο, οι απώλειες από "
    "τον τζόγο θα μπορούσαν να επιδεινώνουν την οικονομική πίεση, ή "
    "ένας τρίτος παράγοντας όπως τα χρέη ή η ανεργία θα μπορούσε να "
    "προκαλεί και τα δύο. Τα έσοδα τζόγου (GGR) δεν αποτελούν ένδειξη "
    "ευημερίας: είναι έσοδο του κλάδου από τις συνολικές απώλειες των "
    "παικτών, όχι διαθέσιμο εισόδημα, και ο υψηλός λόγος GGR προς ΑΕΠ "
    "της Ελλάδας ενισχύεται τεχνητά από το σχετικά μικρό της ΑΕΠ. Η "
    "σύγκριση 16 χωρών της H2 Gambling Capital δεν πρέπει να "
    "αντιμετωπίζεται ως πλήρης κατάταξη της ΕΕ: παραλείπει 11 από τις "
    "27 χώρες-μέλη.",
    "ΕΕΕΠ (Επιτροπή Εποπτείας και Ελέγχου Παιγνίων), έρευνα "
    "συμμετοχής στον τζόγο 2025, διεξήχθη από την Pulse RC.",
    collapse=True)}
...
<p>Το ισχυρότερο επιχείρημα υπέρ βελτίωσης είναι η αγορά
εργασίας. Η ανεργία υποχώρησε από τα ακραία επίπεδα της κρίσης σε κάτω από
9%, ενώ μειώθηκε σημαντικά και η <em>μακροχρόνια ανεργία</em>. Αυτή είναι πραγματική
...
το ποσό που απομένει στο τέλος του μήνα. Με αυτά τα κριτήρια, η οικονομία
των νοικοκυριών δεν ανέκαμψε.</p>
...
είναι απλούστερο από το μοντέλο: <strong><em>ορισμένοι δείκτες
βελτιώθηκαν· το νοικοκυριό δεν ανέκαμψε μαζί τους.</em></strong></p>
...
<details class="fig-methods"><summary>Σημείωση για την αναθεώρηση μεθοδολογίας ανεργίας του 2021</summary>
<p class="fig-caveat">Το 2021, ένας πανενωσιακός κανονισμός (2019/1700)
εναρμόνισε τον τρόπο μέτρησης της ανεργίας σε όλα τα κράτη-μέλη της ΕΕ. Η
ελληνική στατιστική αρχή, όπως και κάθε άλλο κράτος-μέλος, αναθεώρησε
αναδρομικά τη σειρά της έως το 2009 για λόγους συγκρισιμότητας: οι
αναθεωρημένες τιμές είναι υψηλότερες από τις προηγούμενες, όχι χαμηλότερες.
Αυτό το έργο χρησιμοποιεί αυτή τη μία, εναρμονισμένη σειρά σε όλη της την
έκταση, οπότε το παραπάνω μέτρο συσσωρευμένης ανεργίας δεν έχει καμία
ασυνέχεια από την αλλαγή.</p></details>
...
αποδεδειγμένη αιτιώδης διαδρομή μέσα στην Ελλάδα. Πόσο από την ιστορία
παραμένει ανεξήγητο, και πού σταματούν τα στοιχεία αυτού του κειμένου,
είναι αυτό που εξετάζουν οι επόμενες δύο ενότητες.</p>
"""))
...
{context_el('CTX-13', "επιπλέον στοιχεία από άλλη πηγή",
    "Ανεξάρτητη έρευνα (EU-OSHA, εργασιακό άγχος, τρίτος τομέας)",
    '''<p>Μια τρίτη, ανεξάρτητη έρευνα επεκτείνει αυτό το μοτίβο. Σε
    έρευνα της EU-OSHA το 2025 σε εργαζομένους σε 30 ευρωπαϊκές χώρες,
    οι εργαζόμενοι στην Ελλάδα δηλώνουν το υψηλότερο ποσοστό
    εργασιακού άγχους, κατάθλιψης ή ανησυχίας από κάθε άλλη χώρα: 49%,
    έναντι μέσου όρου 29% στις 27 χώρες της ΕΕ. Δεν λύνει το παραπάνω
    ερώτημα· αν κάτι, ένας τρίτος τομέας με την ίδια ακραία εικόνα
    αξίζει την ίδια προσοχή. Είναι, όμως, ένα ακόμη ανεξάρτητο
    εργαλείο στο οποίο η Ελλάδα βρίσκεται στο ένα άκρο.</p>''',
    "Μια τρίτη, ανεξάρτητη πανευρωπαϊκή έρευνα επεκτείνει το μοτίβο "
    "μεταξύ τομέων. Στο εργασιακό άγχος, την κατάθλιψη ή την "
    "ανησυχία, οι εργαζόμενοι στην Ελλάδα δηλώνουν το υψηλότερο "
    "ποσοστό από τις 30 χώρες που συμμετείχαν, 49%, έναντι μέσου "
    "όρου 29% στις 27 χώρες της ΕΕ· διαφορετικό θέμα, διαφορετικός "
    "φορέας και διαφορετικό εργαλείο μέτρησης από τη δική μας "
    "σύγκριση ύφους απάντησης και από τον ίδιο τον δείκτη οικονομικής "
    "δυσκολίας.",
    "Δεν αποτελεί λύση του ζητήματος ύφους απάντησης που αφήνει "
    "ανοιχτό αυτή η ενότητα: το εργασιακό άγχος είναι κι αυτό "
    "δηλωμένο μέγεθος, οπότε μια τάση σε επίπεδο χώρας να δηλώνονται "
    "πιο εύκολα δυσκολίες θα εμφανιζόταν και εδώ, όπως ακριβώς στους "
    "οικονομικούς δείκτες και στην ικανοποίηση από τη ζωή. Δεν "
    "αποτελεί απόδειξη για την αιτία του χάσματος των 52,6 μονάδων: "
    "δεν έγινε κανένας έλεγχος που να το συνδέει με τη δηλωμένη "
    "δυσκολία. Δεν πρέπει να αντιμετωπίζεται μια μεμονωμένη έρευνα "
    "του 2025 της EU-OSHA ως άμεσα συγκρίσιμη ως προς τον σχεδιασμό "
    "με τη δική μας σύγκριση ύφους απάντησης ή με τη σειρά του ESS: "
    "το δείγμα, η διατύπωση των ερωτήσεων και ο πληθυσμός διαφέρουν.",
    'EU-OSHA, Flash Eurobarometer «OSH Pulse 2025», έρευνα πεδίου '
...
{context_el('CTX-10', "συμπληρωματικό στοιχείο", "Άμεσες πληρωμές υγείας",
    '''<p>Στοιχεία της Eurostat δείχνουν ότι το 2023 οι άμεσες πληρωμές των
    ελληνικών νοικοκυριών κάλυπταν το 34,3% της συνολικής τρέχουσας δαπάνης
    υγείας, το τρίτο υψηλότερο ποσοστό στην ΕΕ, πίσω από τη Βουλγαρία και τη
    Λετονία, έναντι μέσου όρου ΕΕ περίπου 14,9%. Ασυνήθιστα μεγάλο μέρος του
    κόστους υγείας στην Ελλάδα φτάνει απευθείας στον οικογενειακό
    προϋπολογισμό αντί να καλύπτεται μέσω δημόσιας χρηματοδότησης ή
    ασφάλισης. Πρόκειται για έναν εύλογο μηχανισμό που θα μπορούσε να
    συμβάλλει στη δηλωμένη δυσκολία, χωρίς αυτή η ανάλυση να τον έχει
    ελέγξει άμεσα.</p>''',
    "Στοιχεία της Eurostat (πίνακας hlth_sha11_hf) δείχνουν ότι το 2023 οι "
    "άμεσες πληρωμές των νοικοκυριών κάλυπταν το 34,3% της τρέχουσας "
    "δαπάνης υγείας στην Ελλάδα, τρίτο υψηλότερο ποσοστό στην ΕΕ πίσω από "
    "τη Βουλγαρία (35,5%) και τη Λετονία (35,1%), έναντι μέσου όρου ΕΕ "
    "περίπου 14,9%. Το εύρημα δείχνει ότι ασυνήθιστα μεγάλο μέρος του "
    "κόστους υγείας στην Ελλάδα φτάνει απευθείας στον οικογενειακό "
    "προϋπολογισμό, αντί να καλύπτεται μέσω δημόσιας χρηματοδότησης ή "
    "ασφάλισης. Πρόκειται για έναν εύλογο μηχανισμό μέσω του οποίου η "
    "οικονομική δυσκολία θα μπορούσε να ενισχύεται, πέρα από όσα εξετάζει άμεσα αυτή "
    "η ανάλυση.",
    "Το ποσοστό αυτό δεν εξετάστηκε στα μοντέλα οικονομικής δυσκολίας "
    "αυτής της ανάλυσης και δεν αποτελεί τεκμηριωμένη εξήγηση του χάσματος "
    "των 52,6 μονάδων. Το 34,3% δεν σημαίνει ότι τα νοικοκυριά δαπανούν το "
    "34,3% του εισοδήματός τους, ούτε ότι το 34,3% των νοικοκυριών "
    "πληρώνει· σημαίνει ότι από κάθε 100 μονάδες τρέχουσας δαπάνης υγείας "
    "στην Ελλάδα, περίπου 34 πληρώνονται απευθείας από τα νοικοκυριά. Ο "
    "μέσος όρος της ΕΕ δεν είναι σταθερός στο 14,9%: η ζωντανή βάση "
    "δεδομένων της Eurostat έχει έκτοτε αναθεωρήσει κάποια από αυτά τα "
    "μεγέθη (ο μέσος όρος της ΕΕ εμφανίζεται πλέον γύρω στο 15,2%, "
    "στοιχείο που δεν επαληθεύτηκε ανεξάρτητα εδώ)· η ελληνική "
    "τιμή και η τρίτη θέση αναφέρονται ως σταθερές και στις δύο εκδόσεις, "
    "αλλά αυτός ο ισχυρισμός βασίζεται σε μία μόνο, μη επαληθευμένη πηγή, "
    "και η ακριβής απόσταση από τον μέσο όρο της ΕΕ εξαρτάται από την "
    "έκδοση δεδομένων που χρησιμοποιείται.",
    'Eurostat, «Healthcare expenditure statistics by function, provider and '
    'financing scheme», στοιχεία έκδοσης Οκτωβρίου 2025 (πίνακας '
    'hlth_sha11_hf). <a href="https://ec.europa.eu/eurostat/'
    'statistics-explained/index.php?title=Healthcare_expenditure_statistics_'
    'by_function,_provider_and_financing_scheme">πηγή</a>')}
...
<p>Ορισμένοι βασικοί δείκτες βελτιώθηκαν. Η ελληνική κοινωνία δεν ανέκαμψε
μαζί τους. Η απασχόληση βελτιώθηκε ουσιαστικά και η πίεση στη στέγαση
υποχώρησε από τα ακραία επίπεδά της. Το κατά κεφαλήν ΑΕΠ, όμως, δεν
επέστρεψε στο επίπεδο του 2008, οι πραγματικοί μισθοί παρέμειναν πολύ
...
και εντάσσονται σε ένα ευρύτερο μοτίβο οικονομικού μειονεκτήματος. Οι
παράγοντες που μπορέσαμε να ελέγξουμε μας βοηθούν να κατανοήσουμε ένα
μεγάλο μέρος της εικόνας, όχι όμως ολόκληρο το χάσμα των 52,6 ποσοστιαίων
μονάδων. Το μέρος που παραμένει ανεξήγητο από τα διαθέσιμα μοντέλα δεν
αποτελεί απόδειξη ελληνικής απαισιοδοξίας. Θεσμική εμπιστοσύνη, πολιτικές
προσαρμογής, φορολογική επιβάρυνση, κατανομή πλούτου, μετανάστευση και
πρόσβαση σε υπηρεσίες υγείας παραμένουν πιθανές πλευρές της εξήγησης,
αλλά δεν τεκμηριώθηκαν εδώ ως αιτίες.</p>
...
<p>Ίσως, λοιπόν, το πρόβλημα δεν είναι ότι τα ελληνικά νοικοκυριά απέτυχαν
να αναγνωρίσουν την ανάκαμψη. Είναι ότι η δημόσια συζήτηση ονόμασε
«ανάκαμψη» τη βελτίωση ορισμένων δεικτών, πριν αποκατασταθούν το βιοτικό
επίπεδο, η αγοραστική δύναμη και η οικονομική ασφάλεια της κοινωνίας. Αυτή
η τελική σύνθεση είναι ερμηνεία των συγγραφέων και όχι νέο εμπειρικό
αποτέλεσμα: προκύπτει άμεσα από όσα έδειξε η ανάλυση ότι χάνει ο AROP από
μόνος του, όχι από κάποιον επιπλέον έλεγχο. Το πολιτικό σημείο που
προκύπτει από αυτά τα ευρήματα είναι απλό: δεν μπορούμε να επικαλούμαστε
τους δείκτες για να πιστοποιούμε την ανάκαμψη και, όταν η εμπειρία των
νοικοκυριών διαφωνεί, να μεταφέρουμε αυτόματα την ευθύνη στην ψυχολογία
και στις επιλογές τους.</p>
...
<title>{_TITLE_EL}</title>
<meta name="description" content="Δύο στα τρία ελληνικά νοικοκυριά δηλώνουν δυσκολία στην κάλυψη των αναγκών τους, ενώ ο επίσημος δείκτης φτώχειας φαίνεται πολύ λιγότερο ακραίος. Όλη η αφήγηση πίσω από αυτό το χάσμα.">
<meta property="og:type" content="article">
<meta property="og:title" content="{_TITLE_EL}">
<meta property="og:description" content="Δύο στα τρία ελληνικά νοικοκυριά δηλώνουν δυσκολία στην κάλυψη των αναγκών τους, ενώ ο επίσημος δείκτης φτώχειας φαίνεται πολύ λιγότερο ακραίος.">
<meta property="og:url" content="https://parastelios.github.io/greece-poverty-analysis/narrative_el.html">
<meta property="og:image" content="https://parastelios.github.io/greece-poverty-analysis/assets/carousel/og-el.png">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:locale" content="el_GR">
<meta property="og:locale:alternate" content="en_US">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{_TITLE_EL}">
<meta name="twitter:description" content="Δύο στα τρία ελληνικά νοικοκυριά δηλώνουν δυσκολία στην κάλυψη των αναγκών τους, ενώ ο επίσημος δείκτης φτώχειας φαίνεται πολύ λιγότερο ακραίος.">
<meta name="twitter:image" content="https://parastelios.github.io/greece-poverty-analysis/assets/carousel/og-el.png">{BASE}
<style>{ce.CSS}
{ce.TERM_CSS}
...
<h1>{_TITLE_EL}</h1>
<p class="standfirst">Η αγορά εργασίας της Ελλάδας δείχνει να ανακάμπτει. Τα νοικοκυριά της όχι.
Η απόσταση ανάμεσα στην εισοδηματική φτώχεια και τη δηλωμένη

...
(function(){
  ...
  function labelGutter(host,labels,min,max){
    ...
    // Supported rows draw bold, and bold is wider. Measuring at the regular
    // weight left the boldest label clipped by its first two characters.
    txt.setAttribute('style','font-weight:700');
    probe.appendChild(txt); host.appendChild(probe);
    let widest=0;
    for(const l of labels){
      txt.textContent=String(l);
      widest=Math.max(widest,txt.getComputedTextLength()||0);
    }
    host.removeChild(probe);
    // getComputedTextLength returns 0 when the chart mounts without layout --
    // inside a closed <details>, an inactive view tab, or a collapsed pane.
    // Trusting that would hand back the minimum gutter, which is NARROWER than
    // the fixed value this replaced, turning an improvement into a regression.
    // Fall back to an estimate calibrated on the bold label font (~7.6px/char).
    if(!(widest>0)){
      const chars=Math.max(0,...labels.map(l=>String(l).length));
      widest=chars*7.6;
    }
    return Math.min(max,Math.max(min,Math.ceil(widest)+16));
  }

  // Shorten a rendered label until it fits, keeping the full text reachable as
  // a tooltip. Sizing the gutter to the longest label is not enough on its own:
  // when the width cap binds, the surplus has to come off the text.
  function fitLabel(node,full,maxW){
    node.textContent=full;
    const measured=node.getComputedTextLength();
    // Same guard: an unmeasurable label is left intact rather than truncated
    // against a bogus zero.
    const wide=measured>0?measured:String(full).length*7.6;
    if(maxW<=0||wide<=maxW){return node;}
    if(!(measured>0)){
      const fit=Math.max(1,Math.floor(maxW/7.6)-1);
      if(fit<full.length){
        const ttl=document.createElementNS('http://www.w3.org/2000/svg','title');
        ttl.textContent=full;
        node.textContent=full.slice(0,fit).trimEnd()+'\u2026';
        node.appendChild(ttl);
      }
      return node;
    }
    const title=document.createElementNS('http://www.w3.org/2000/svg','title');
    title.textContent=full; node.appendChild(title);
    let lo=0,hi=full.length;
    while(lo<hi){
      const mid=Math.ceil((lo+hi)/2);
      node.firstChild.nodeValue=full.slice(0,mid)+'\u2026';
      if(node.getComputedTextLength()<=maxW){lo=mid;}else{hi=mid-1;}
    }
    node.firstChild.nodeValue=full.slice(0,lo).trimEnd()+'\u2026';
    return node;
  }
  ...
  function panel(host,d){
    ...
    const padR=labelGutter(host,(d.series||[]).map(s=>s.label||''),52,W*0.34);
    ...
    ends.forEach(e=>{
      ...
      svg.appendChild(fitLabel(t,e.s.label,W-(e.x+4)-2));});
  ...
  function coefficient(host,d){
    ...
    const padL=labelGutter(host,d.rows.map(r=>r.label),W<480?110:150,W*0.42);
    const padR=labelGutter(host,d.rows.map(r=>r.right||''),W<480?54:96,W*0.26);
    ...
    d.rows.forEach((r,i)=>{
      ...
      svg.appendChild(fitLabel(lbl,r.label,padL-14));
      const vt=el('text',{x:W-padR+8,y:y+4,class:'axis-label',
        style:`fill:var(--${tone});font-weight:600`});
      svg.appendChild(fitLabel(vt,r.right||'',padR-12));
    });
    host.insertBefore(svg,host.firstChild);
  ...
  function ladder(host,d){
    ...
    const padL=labelGutter(host,d.rows.map(r=>r.label),W<480?42:54,W*0.42);
    ...
    d.rows.forEach((r,i)=>{
      ...
      svg.appendChild(fitLabel(lb,r.label,padL-10));
  ...
  function dumbbell(host,d){
    ...
    const padL=labelGutter(host,d.rows.map(r=>r.label),W<480?104:150,W*0.42);
    const padR=labelGutter(host,d.rows.map(r=>r.right||''),W<480?52:92,W*0.26);
    ...
    const marks=[];
    d.rows.forEach((r,i)=>{
      const y=padT+i*rowH+rowH/2, tone=r.tone||'text-muted';
      svg.appendChild(el('line',{x1:xs(r.a),x2:xs(r.b),y1:y,y2:y,
        stroke:`var(--${tone})`,'stroke-width':2.5,opacity:.45}));
      svg.appendChild(el('circle',{cx:xs(r.a),cy:y,r:4,fill:'var(--text-muted)'}));
      svg.appendChild(el('circle',{cx:xs(r.b),cy:y,r:5.5,fill:`var(--${tone})`}));
      const lb=el('text',{x:padL-10,y:y+4,'text-anchor':'end',class:'axis-label',
        style:r.strong?'font-weight:700':''});
      svg.appendChild(fitLabel(lb,r.label,padL-14));
      if(r.right){const rt=el('text',{x:W-padR+8,y:y+4,class:'axis-label',
        style:`fill:var(--${tone});font-weight:600`});
        svg.appendChild(fitLabel(rt,r.right,padR-12));}
      marks.push({y:y,r:r});});
    host.insertBefore(svg,host.firstChild);
    if(d.legendA||d.legendB){
      const lg=document.createElement('div');lg.className='legend';
      lg.innerHTML=`<span class="lg-item"><svg width="12" height="12" aria-hidden="true">
        <circle cx="6" cy="6" r="4" fill="var(--text-muted)"/></svg>${d.legendA||'start'}</span>
  ...
  function heatmap(host,d){
    ...
    const labW=labelGutter(host,d.rows.map(r=>r.label),W<480?96:150,W*0.40);
    ...
    d.rows.forEach((r,i)=>{
      ...
      svg.appendChild(fitLabel(lb,r.label,labW-12));

...
(function(){
  ...
  function strip(host,d){
    ...
    const rowH=W<480?58:64, padR=W<480?54:72, padT=14;
    ...
    d.strips.forEach((s,i)=>{
      ...
      [[lo,'start'],[hi,'end']].forEach(([v,anch],k)=>{
        ...
        svg.appendChild(tx);});
      const dir=el('text',{x:(labW+W-padR)/2,y:y+20,'text-anchor':'middle',
  ...
  function dumbbell(host,d){
    ...
    d.rows.forEach((r,i)=>{
      ...
      svg.appendChild(outlineNeutral(el('circle',{cx:xs(r.a),cy:y,r:4,
        fill:'var(--chart-neutral)'}),'chart-neutral'));
      svg.appendChild(el('circle',{cx:xs(r.b),cy:y,r:5.5,fill:'var(--chart-gr)'}));
    ...
    if(d.legendA||d.legendB){
      ...
      lg.innerHTML=`<span class="lg-item"><svg width="12" height="12" aria-hidden="true">
        ...
        <circle cx="6" cy="6" r="5" fill="var(--chart-gr)"/></svg>${d.legendB||'end'}</span>`;
      host.insertBefore(lg,svg.nextSibling);}
    const tp=tip(host);let idx=-1;
    const show=i=>{if(i<0||i>=marks.length)return;idx=i;const r=marks[i].r;
